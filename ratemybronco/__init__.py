...
ratemybroncoDB = mysql.connector.connect(
    option_files="ratemybronco/config.cfg", option_groups="database")
...
def search():
    ...
    if query:
        ...
        if instructor:
            ...
            mycursor.execute("SELECT * FROM instructor i WHERE i.firstName=%s AND i.lastName=%s",
                             (parsed_query[0], parsed_query[1],))
        else:
            mycursor.execute(
                "SELECT * FROM class c WHERE c.CourseNumber=%s", (query,))
